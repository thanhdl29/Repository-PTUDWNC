	...
	public class PostValidator : AbstractValidator<PostEditModel>
	{
		...
		public PostValidator(IBlogRepository blogRepository)
		{
			...
			When(x => x.Id <= 0, () =>
			{
				...
				.WithMessage("Bạn phải chịn hình ảnh cho bài viết");
			})
				.Otherwise(() =>
				{
					RuleFor(x => x.ImageFile)
		...
		// Kiểm tra xem bài viết đã có hinhaf ảnh chưa 
			...
			CancellationToken cancellationToken)
		{
			...
			// Nếu bài viết đã có hình ảnh => không chọn file
			if (!string.IsNullOrWhiteSpace(post?.ImageUrl))
				return true;

			//Ngược lại (bài viết chưa có hình ảnh), kiểm tra xem người dùng đã chịn file hay chưa, Nếu chưa thì bào lỗi)
